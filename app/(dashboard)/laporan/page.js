...
import { products, rupiah, sales, stockStatus } from '@/lib/demo-data';

function downloadSales() {
    const esc = (v) => `"${`${v}`.replace(/"/g, '""')}"`;
    const csv = ['Hari;Total Penjualan (IDR)', ...sales.map(s => `${esc(s.day)};${s.total}`)].join('\n');
...
export default function Page() { return <div className="flex flex-col gap-5"><PageHeader title="Laporan" description="Analisis operasional dan kinerja DIAN MOTOR."> <Button onClick={downloadSales}><Download data-icon="inline-start"/>Unduh Laporan</Button></PageHeader>
