...
import { setUnread } from '@/lib/notif-store';
...
export default function Page() {
    ...
    return <div className="flex flex-col gap-5"><PageHeader title="Notifikasi" description="Pembaruan penting operasional bengkel."> <Button variant="outline" onClick={markAll} disabled={!unread}><CheckCheck data-icon="inline-start"/>Tandai Semua Dibaca</Button></PageHeader>
    <div className="mx-auto w-full max-w-3xl overflow-hidden rounded-[17px] border border-border bg-card shadow-sm">
      <div className="flex items-center justify-between border-b border-border px-5 py-3"><p className="font-mono text-[10px] font-bold tracking-[.12em] text-muted-foreground">{unread} BELUM DIBACA</p><span className="flex items-center gap-2 rounded-full bg-emerald-500/10 px-2.5 py-1 font-mono text-[9px] font-bold text-emerald-400"><span className="size-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,.7)]"/>LIVE</span></div>
