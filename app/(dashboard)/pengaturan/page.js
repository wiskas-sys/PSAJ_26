...
import { Building2, Server, ShieldCheck, Wrench } from 'lucide-react';
...
export default function Page() {
    ...
    return <div className="flex flex-col gap-5"><PageHeader eyebrow="SYSTEM SETTINGS" title="Pengaturan" description="Kelola profil bengkel dan preferensi sistem."></PageHeader>
    <Tabs defaultValue="business" className="gap-4"><TabsList variant="line" className="h-auto w-fit max-w-full gap-1 overflow-x-auto rounded-xl bg-card p-1 shadow-sm ring-1 ring-foreground/10"><TabsTrigger className={tabCls}>Informasi Bisnis</TabsTrigger><TabsTrigger className={tabCls}>Preferensi Sistem</TabsTrigger></TabsList>
      <TabsContent value="business" className="mt-4"><div className="grid items-start gap-4 xl:grid-cols-2">
        ...
        <div className="grid gap-4">
          <Card className="rounded-[17px]"><CardHeader className="border-b border-border"><CardTitle className="flex items-center gap-2"><Building2 className="size-4 text-primary"/>Lokasi Operasional</CardTitle></CardHeader><CardContent className="grid gap-3 p-5">{[['TERM-01 · Jakarta Pt.', 'Kantor pusat & bengkel utama', Wrench], ['TERM-02 · Purwokerto', 'Cabang ekspansi wilayah tengah', Wrench], ['TERM-03 · Semarang', 'Cabang ekspansi wilayah utara', Wrench]].map(([name, desc, Icon]) => <div key={name} className="flex items-center gap-3 rounded-xl border border-border p-3"><span className="grid size-9 shrink-0 place-items-center rounded-[10px] bg-muted text-primary"><Icon className="size-4"/></span><div className="min-w-0 flex-1"><strong className="block truncate text-[13px]">{name}</strong><small className="block text-[11px] text-muted-foreground">{desc}</small></div><span className="stock-chip">ONLINE</span></div>)}</CardContent></Card>
          <Card className="rounded-[17px]"><CardHeader className="border-b border-border"><CardTitle className="flex items-center gap-2"><Server className="size-4 text-primary"/>Kesehatan Sistem</CardTitle></CardHeader><CardContent className="grid gap-2 p-5">{[['Supabase Database', 'Terhubung · RLS aktif'], ['Auth Service', 'OK · 128 ms'], ['Storage & Sinkronisasi', 'OK · real-time']].map(([name, st]) => <div key={name} className="flex items-center justify-between border-b border-border py-2.5 last:border-0"><span><strong className="block text-[13px]">{name}</strong></span><small className="font-mono text-[10px] text-emerald-400">{st}</small></div>)}</CardContent></Card>
        </div>
      </div></TabsContent>
      <TabsContent value="system" className="mt-4"><div className="grid items-start gap-4 xl:grid-cols-2">
        ...
        <Card className="rounded-[17px]"><CardHeader className="border-b border-border"><CardTitle className="flex items-center gap-2"><ShieldCheck className="size-4 text-primary"/>Keamanan Sesi</CardTitle></CardHeader><CardContent className="grid gap-3 p-5">{[['Sesi terminal aktif', 'TERM-01 · Shift 08.00–16.00'], ['Autentikasi', 'Supabase Auth · 2FA tersedia'], ['Token kedaluwarsa', 'Aman · sesi diperbarui otomatis']].map(([name, st]) => <div key={name} className="flex items-center justify-between rounded-xl border border-border p-3"><span className="text-[13px] font-semibold">{name}</span><small className="max-w-[55%] truncate font-mono text-[10px] text-muted-foreground">{st}</small></div>)}<Button variant="outline" className="h-10" onClick={() => toast.success('Sesi diperbarui.')}>Perbarui Sesi</Button></CardContent></Card>
      </div></TabsContent>
    </Tabs>
  </div>;
}